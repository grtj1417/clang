...
int is_mixed_number(char str)
{

    char *left_paren = strchr(str, '(');
    char *right_paren = strchr(str, ')');
    if (left_paren == NULL || right_paren == NULL) return 0;
    ...
    if (numerator_num >= denominator_num || denominator_num == 0) return 0;
    return 1;
}
int main() {
    int n = 0;
    ...
    while (n--) {
        ...
        if (state == Var) {
            ...
            if(is_mixed_number(str[21]) == 0) printf("error");
